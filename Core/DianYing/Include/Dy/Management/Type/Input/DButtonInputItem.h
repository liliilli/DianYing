...
/// @class DButtonInputItem
/// @brief Button status binder. Axis and Action must be retrieve values from this.
class DButtonInputItem final
{
public:
  /// @brief Update status with from library.
  void Update(int iLowLevelStatus) noexcept;

  /// @brief Get status value on present frame.
  MDY_NODISCARD EInputButtonStatus Get() const noexcept;

private:
  EInputButtonStatus mStatus = EInputButtonStatus::Released;
};
