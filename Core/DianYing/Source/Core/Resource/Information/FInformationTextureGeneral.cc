...
FInformationTextureGeneral::FInformationTextureGeneral(_MIN_ const PDyTextureInstanceMetaInfo& metaInfo) :
    FInformationTexture(metaInfo)
{
  ...
  { // But external, read buffer from file, get width, height and image format and convert raw buffer to buffer type.
    ...
    const auto dataBuffer = std::make_unique<DImageBinaryBuffer>(std::get<std::string>(metaInfo.mExternalFilePath));
...
const std::vector<TU8>& FInformationTextureGeneral::GetBuffer() const noexcept
{
  return this->mTextureImageBuffer;
}

const DIVec2& FInformationTextureGeneral::GetSize() const noexcept
{
