...
FInformationModelSkeleton::FInformationModelSkeleton(_MIN_ const PDyModelSkelInstanceMetaInfo& metaInfo) :
...
const std::string& FInformationModelSkeleton::GetSpecifierName() const noexcept
{
  return this->mSpecifierName;
}

TU32 FInformationModelSkeleton::GetNodeCount() const noexcept
{
  return static_cast<TU32>(this->mSkeletonInfo.mExportedSkeleton.size());
}

TU32 FInformationModelSkeleton::GetInputBoneCount() const noexcept
{
  return static_cast<TU32>(this->mSkeletonInfo.mBoneOffsetList.size());
}

const DSkeletonBone& FInformationModelSkeleton::GetRefSkeletonNode(_MIN_ TU32 iIndex) const noexcept
{
...
std::vector<TU32> FInformationModelSkeleton::GetChildrenNodeIdList(_MIN_ TI32 iIndex) const noexcept
{
...
const DMat4& FInformationModelSkeleton::GetRootInverseTransform() const noexcept
{
  return this->mSkeletonInfo.mSkeletonRootInverseTransform;
}

const DMat4& FInformationModelSkeleton::GetOffsetMatrixOfBone(_MIN_ TU32 iIndex) const noexcept
{
  return this->mSkeletonInfo.mBoneOffsetList[iIndex].mBoneOffsetMatrix;
}

const DSkeletonBone& FInformationModelSkeleton::GetRefSkeletonNodeFromBoneOffsetId(_MIN_ TU32 iIndex) const noexcept
{
  return this->mSkeletonInfo.mExportedSkeleton[this->mSkeletonInfo.mBoneOffsetList[iIndex].mIndexSkeletonNode];
}

const TI32 FInformationModelSkeleton::GetSkeletonNodeIdFromBoneOffsetId(TU32 iIndex) const noexcept
{
  return this->mSkeletonInfo.mBoneOffsetList[iIndex].mIndexSkeletonNode;
}

const std::vector<DBoneOffset>& FInformationModelSkeleton::GetOffsetBoneList() const noexcept
{
