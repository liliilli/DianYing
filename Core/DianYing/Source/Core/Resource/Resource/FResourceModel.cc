...
FResourceModel::FResourceModel(const FInformationModel& input) :
...
TU32 FResourceModel::GetNumberOfMaterial() const noexcept
{
  return this->mMaterialResource.size();
}

TDyResourceBinderMaterial* FResourceModel::GetMaterialBinder(_MIN_ TU32 iIndex) noexcept
{
  // Check bound
