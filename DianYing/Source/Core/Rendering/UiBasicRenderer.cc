...
dy::DDyColor     uTextColor    = dy::DDyColor{1, 1, 0, 1};
...
GetCharacterVertices(_MIN_ const dy::DDyFontCharacter& ch_info, _MIN_ const dy::DDyVector2& position, _MIN_ const TF32 scale)
{
  const auto xOffset      = ch_info.mBearing.X * scale;
  const auto yOffset      = (ch_info.mSize.Y - ch_info.mBearing.Y) * scale;
  const auto charPos      = position + dy::DDyVector2{ xOffset, -yOffset };

  const auto w = ch_info.mSize.X * scale;
  const auto h = ch_info.mSize.Y * scale;

  const auto l = charPos.X;
  const auto r = charPos.X + w;
  const auto t = charPos.Y + h;
...
FDyUIBasicRenderer::FDyUIBasicRenderer()
{
  ...
  uUiProjMatrix = glm::ortho(
      ...
      static_cast<float>(overallScreenHeight) / 2,
      0.2f, 10.0f);
  sSampleShaderPtr  = MDyHeapResource::GetInstance().GetShaderResource(MSVSTR(builtin::FDyBuiltinShaderGLRenderDefaultFont::sName));
...
void FDyUIBasicRenderer::RenderScreen()
{
  ...
  glUniformMatrix4fv(glGetUniformLocation(sSampleShaderPtr->GetShaderProgramId(), "uUiProjMatrix"), 1, GL_FALSE, &uUiProjMatrix[0].X);
  ...
  for (const TC16& ucs2Char : textComponent->GetText()) {
    ...
    if (ucs2Char == '\n')
    {
      ...
      renderPosition.Y -= container.GetScaledLinefeedHeight(scale);
      continue;
    }
    ...
    Render(charInfo, GetCharacterVertices(charInfo, renderPosition, scale));
    renderPosition.X += (charInfo.mAdvance >> 6) * scale;
  }
