#ifndef GUARD_DY_CORE_REFLECTION_RDyGlobalInstanceManager_H
#define GUARD_DY_CORE_REFLECTION_RDyGlobalInstanceManager_H
///
/// MIT License
...
/// @class RDyGlobalInstanceManager
...
public:
  ...
  [[nodiscard]] static typename TDyGlobalInstance<THashVal>::Type* GetGlobalInstance() noexcept
  {
    ...
    auto& map = RDyGlobalInstanceManager::GetInstanceMap();
    const auto it = map.find(THashVal);
    if (it == map.end()) { return nullptr; } else { return static_cast<TConversionType*>(it->second.get()); }
  }
};

/// @struct RDyGlobalInstanceRegistration
/// @brief Global intance registration type.
template <typename TType>
struct RDyGlobalInstanceRegistration final : public RDyGlobalInstanceManager
{
  inline static bool mFlag = false;
  RDyGlobalInstanceRegistration(hash::THashVal32 iVal)
  {
...
#define MDY_GET_GLOBALINSTANCE(__MAStringLiteral__) \
  ::dy::reflect::RDyGlobalInstanceManager::GetGlobalInstance<::dy::hash::DyToCrc32Hash(MDY_TO_STRING(__MAStringLiteral__))>()

} /// ::dy::reflect namespace

#endif /// GUARD_DY_CORE_REFLECTION_RDyGlobalInstanceManager_H