...
class DCompUpdator final 
{
public:
  using TUnaryPredicate = std::function<void(const TType&)>;

  DCompUpdator() = default;
  DCompUpdator(const TType& iValue, TUnaryPredicate mCallback) : mValue{iValue}, mCallbackFunction{ std::move(mCallback) } {};

  /// @brief Set value
  DCompUpdator& operator=(const TType& iNewValue)
  {
    // Check validation
...
#endif /// GUARD_DY_Helper_Type_DCompUpdator_H