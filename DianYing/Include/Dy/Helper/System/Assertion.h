...
///
inline void __EnhancedAssert( const char* expression,  bool result,  const char* filePath,  int line,  const char* failedMessage)
{
  if (result == false)
  {
    {
      const auto msg = fmt::format(R"dy(Assert Failed : {0}, Expected : {1}, Sourtce : {2}, Line : {3})dy",
