...
public:
  ...
  ///
  void SetLocalSpacePosition(const DVector3& localPosition) noexcept;

	///
	/// @brief Set world position.
	/// @param[in] worldPosition Winal position in Screen space and from parent' object.
	///
	void SetWorldSpacePosition(const DVector3& worldPosition) noexcept;

  ///
  /// @brief The method set final position but keeping parent position
  /// but adjusting FDyPawn's world position.
  ///
  void SetWorldPositionFromFinalPosition(const DVector3& finalPosition) noexcept;

  ///
  /// @brief Add offset value with axis as local position.
  ///
  void AddOffsetLocalSpacePosition(EAxis3D axis, float value) noexcept;

  ///
  /// @brief Add offset value with axis as world position.
  ///
  void AddOffsetWorldSpacePosition(EAxis3D axis, float value) noexcept;
  ...
  ///
  void SetLocalEulerAngle(const DVector3& eulerAngleValue) noexcept;
  ...
  /// @param[in] eulerAngleValue Euler angle value to set on.
  ///
  void SetWorldEulerAngle(const DVector3& eulerAngleValue) noexcept;

  ///
  /// @brief Add offset eulerAngleValue with axis as local rotation angle.
  ///
  void AddLocalEulerAngle(EAxis3D axis, float eulerAngleValue) noexcept;

  ///
  /// @brief Add offset eulerAngleValue with axis as world rotation angle.
  ///
  void AddWorldEulerAngle(EAxis3D axis, float eulerAngleValue) noexcept;
  ...
  ///
  [[nodiscard]] const DMatrix4x4& GetTransform() const noexcept;
...
private:
  ...
  DVector3 mLocalEulerAngle;
  DVector3 mWorldEulerAngle;
  ...
  DVector3 mLocalScale = DVector3{ 1.f };
  /// Scale world factor. default is (1, 1, 1)
  DVector3 mWorldScale = DVector3{ 1.f };
  ...
  MDY_TRANSIENT DMatrix4x4 mFinalTransform;
