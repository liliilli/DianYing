...
/// @class FFontContainerRuntime
/// @brief `ELoadingType` is `::Runtime`.
class FFontContainerRuntime : public AFontContainer
{
public:
  FFontContainerRuntime(const PDyMetaFontInformation::DRuntime& details);

  /// @brief Release font container resources.
  virtual ~FFontContainerRuntime();

  MDY_ONLY_MOVEABLE_PROPERTIES_DEFAULT(FFontContainerRuntime);
