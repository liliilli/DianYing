...
{
  SECTION("Check TI32 vector values are matched.")
  {
  ...
  SECTION("Check TI32 vector values has arithmetic operators")
  {
    DVector4<TI32> vector{0, 1, 2, 3};
  ...
  SECTION("Check TF32 vector values are matched.")
  {
    DVector4<TF32> vector{0, 1, 2, 3};
    REQUIRE( vector.X == 0 );
    REQUIRE( vector.Y == 1 );
    REQUIRE( vector.Z == 2 );
    REQUIRE( vector.W == 3 );
  ...
  SECTION("Check TF32 vector values has arithmetic operators")
  {
    DVector4<TF32> vector{0, 1, 2, 3};
    vector += DVector4<TF32>(-5, -4, -3, -2);
    REQUIRE(vector[0] == -5); REQUIRE(vector[1] == -3); REQUIRE(vector[2] == -1); REQUIRE(vector[3] == +1);
    ...
    vector *= 3;
    vector.X += 1;
    REQUIRE(vector[0] == 1); REQUIRE(vector[1] == 3); REQUIRE(vector[2] == 6); REQUIRE(vector[3] == 9);
    
    const auto homolen  = vector.GetHomogeneousLength();
    const auto square   = vector.GetSquareLength();
    REQUIRE(square == 1 + 9 + 36 + 81);
    REQUIRE(homolen == std::sqrt(1.f + 9.f + 36.f));
    const auto len = vector.GetLength();
    REQUIRE(len == std::sqrt(1 + 9 + 36 + 81.f));
